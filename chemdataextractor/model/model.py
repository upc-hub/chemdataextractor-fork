...
class Compound(BaseModel):
    ...
    hein_htets = ListType(ModelType(HeinHtet))
    #ammonia_points = ListType(ModelType(AmoniaRate))
...
'''
'''
class My_Current_Density_Model(CurrentDensityModel):
    specifier = StringType(parse_expression=(R('m?A')).add_action(join))
    compound = ModelType(Compound, required=True, contextual=True)
    #AmmoniaRate = ModelType(AmoniaRate, required=True, contextual=True)
    parsers = [AutoSentenceParser(), QuantityModelTemplateParser(), MultiQuantityModelTemplateParser(), BpownParser()]
'''
