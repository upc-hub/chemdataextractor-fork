...
prefix = (I(u'current')+I(u'density')+I(u'of')|I(u'current')+I(u'density')|I(u'density')+I(u'of')+I(u'current')).hide()
units = ((R('[mA]'))+(R('[cm−2]')))(u'units').add_action(join)
value = (Optional(R('^[~∼\<\>]$')) + Optional(R('^[\-–−]$')) + R('^[\+\-–−]?\d+(\.\d+)?$'))('value').add_action(merge)
new_property = (prefix + value + units)(u'new_property')


class CurrentTestParser(BaseSentenceParser):
    root = new_property
